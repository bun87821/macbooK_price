...
2. **GitHub Actions（`.github/workflows/check.yml`）— 每 30 分鐘的商品檢查**
   跟 Worker 的 `/subs` 端點拿所有訂閱 → Playwright 渲染 [整修品頁面](https://www.apple.com/tw/shop/refurbished/mac) → 對每個人的條件比對標題、必要時抓商品頁確認容量 → 新上架的發 Telegram 通知 → 把「已通知過」記錄存回 `seen.json`。

`/subs` 是公開端點，但它回傳的內容與本來就每輪 commit 進這個 public repo 的 `subscriptions.json` 快照完全相同，沒有額外暴露；也因此 Actions 不需要任何 Cloudflare 憑證。`/subs` 讀不到時（暫時故障），Actions 會退回使用快照，通知功能不中斷。
...
1. 把 `check_refurb.py` 開頭的 `SUBS_URL` 換成自己的 Worker 網址（`https://<你的worker網址>/subs`）
2. Repo 頁面 → Settings → Secrets and variables → Actions → New repository secret，只需要一個：
